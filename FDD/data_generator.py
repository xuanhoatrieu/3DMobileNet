...
def read_imgs(start_frame, end_frame, rgb_folder, mhi_folder):
    rgb_clip = []
    mhi_clip = []

    for i in range(start_frame, end_frame):
        rgb_file = os.path.join(rgb_folder, str(i + 1).zfill(5) + ".jpg")
        mhi_file = os.path.join(mhi_folder, str(i + 1).zfill(5) + ".jpg")
        rgb_img = Image.open(rgb_file)
        rgb_img = rgb_img.resize((224,224))
        rgb_img = np.asarray(rgb_img) / 255.0
        rgb_clip.append(rgb_img)

        mhi_img = Image.open(mhi_file)
        mhi_img = mhi_img.resize((224,224))
        mhi_img = np.asarray(mhi_img) / 255.0
        mhi_clip.append(mhi_img)

    return rgb_clip, mhi_clip
...
def generator_data(data, batch_size):
    while True:
        ...
        for _ in range(batch_size):
            ...
            mhi_folder = row['mhi_folder']
            rgb_clip, mhi_clip = read_imgs(start_frame, end_frame, rgb_folder, mhi_folder)
            X_rgb.append(rgb_clip)
            X_mhi.append(mhi_clip)
            y.append(row['label'])
        X_rgb = np.asarray(X_rgb)
        X_mhi = np.asarray(X_mhi)
        y = np.asarray(y)
        yield [X_rgb, X_mhi], y

def test_data(data, batch_size=32, start_idx=0):
    X_rgb = []
    X_mhi = []
    y = []
